...
export function GradientHeroCard({
  ...
  const colorPair = theme.gradients[gradient];
  ...
  return (
    <LinearGradient
      ...
      // Solid bg (under the gradient) lets RN compute the shadow path efficiently.
      style={[styles.base, shadow, { backgroundColor: colorPair[0] }, style]}
    >
      {tag ? (
...
const buildStyles = (t: ReturnType<typeof useTheme>) =>
  StyleSheet.create({
    ...
    cardName: { color: t.palette.white, fontFamily: fontFamily.semibold, fontSize: scaleFont(12), fontWeight: "600" },
