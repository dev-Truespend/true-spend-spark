...
> **Purpose:** This document is the single source of truth for rebuilding TrueSpend from its current over-engineered 119-function legacy AI scaffold into a lean, AI-agent-first credit card optimization platform. Hand this document to Claude Code as the primary build prompt.
...
### The problem
The project was scaffolded by legacy AI generator. Every feature request caused the AI to add more edge functions without removing old ones. The result is **119 Supabase edge functions** — an enterprise microservices stack for a pre-revenue startup.
...
- [ ] Remove legacy AI gateway wrappers from `ai-analyze-spending`
...
#### Acceptance criteria
- User visits Insights page → sees AI analysis generated by Claude, not legacy Gemini gateway
...
# Remove old AI gateway from spending analysis (was calling legacy Gemini gateway)
