...
#   docker build -f docker/truespend.eventconsumer.Dockerfile --target development -t truespend-eventconsumer:dev service/
#   docker run --rm -v "$PWD/service":/src -e DOTNET_ENVIRONMENT=Development truespend-eventconsumer:dev
#
# Production (push to ACR):
#   docker build -f docker/truespend.eventconsumer.Dockerfile --target production -t truespend-eventconsumer:prod service/
...
CMD ["dotnet", "watch", \
     "--project", "truespend.eventconsumer/truespend.eventconsumer.csproj", \
     "--no-launch-profile", \
     "run"]
...
ENTRYPOINT ["dotnet", "truespend.eventconsumer.dll"]
