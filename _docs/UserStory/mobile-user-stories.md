...
- User can manually log a transaction from the Transactions sub-tab via the floating `+` action (`6.1`, `6.1b`)
- User can enter merchant, amount, card, date, time, location, and category when manually logging a transaction (`6.1b`)
- User can pick the card used for a manually logged transaction from their linked + manual cards (`6.1b`, `6.2`)
- User can see a reward check while logging a manual transaction if a better card appears available (`6.1b`)
- User can view recent manually logged transactions on mobile (`6.1`)
...
- User can see earned points or cash back computed from card rules for each manually logged transaction (`6.1`)
- User can see missed rewards on a manually logged transaction (`6.1`, `6.2`)
...
- User can view rewards earned over week, month, quarter, and year from transactions (`6.3`)
...
- User can see AI-generated reward optimization insights powered by Azure OpenAI (`6.3`)
- User can opt out of personalized AI insights via the Privacy toggle (`8.4`, `6.3`)
...
- User can receive a lock-screen card nudge when arriving at a known merchant, powered by Foursquare background geofencing (`P2.7b`, lock screen, `7.2`)
- User can navigate from the lock-screen nudge to the related recommendation/notification detail (`P2.7b`, `7.2`)
- User can opt out of geo-arrival nudges by disabling the best-card-alert notification type (`7.3`)
...
- User can change profile photo by uploading their own; defaults to provider photo (Google/Apple) (`8.2`)
...
- User can open Stripe-hosted checkout to start or upgrade a paid plan (`8.3`, `2.5`)
- User can use Apple Pay from Stripe-hosted checkout on iOS when available (`8.3`, `2.5`)
- User can use Google Pay from Stripe-hosted checkout on Android when available (`8.3`, `2.5`)
- User can manage an existing subscription from mobile via the Stripe customer portal (`8.3`)
- User can cancel a subscription from billing settings via the Stripe customer portal (`8.3`)
- User can update or remove a payment method via the Stripe customer portal (`8.3`)
- User can see entitlement changes reflected in the app shortly after Stripe webhook processing (`8.3`, `5.1`)
- User can return from Stripe checkout to an in-app confirmation state (`8.3`, `2.5`)
