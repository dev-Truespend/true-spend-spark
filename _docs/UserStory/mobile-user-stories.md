...
Phase 1 covers screens `1.1` through `8.5`: auth, onboarding, in-store recommendations, cards, the Insights tab (manual transactions only), notifications, profile, billing, privacy, and sync.

Scope notes for Phase 1:

- Plaid is used for card metadata only — no transaction import.
- The Phase 1 bottom tab bar is: Home, Cards, Insights, Profile. The Search tab (online purchase helper) ships in Phase 2.
- Receipt scanning, budgets, geofence management, and the geofence-triggered lock-screen nudge (`P2.7b`) are Phase 2.
- Billing is processed by Stripe via hosted checkout and customer portal; checkout should surface Apple Pay on iOS and Google Pay on Android when available. AI insights are generated by Azure OpenAI.
...
Phase 1 scope note: Plaid is used for card metadata only — no transaction import. The only transaction source in Phase 1 is manual entry. Receipt-derived transactions arrive in Phase 2.
...
- User can view rewards earned over week, month, quarter, and year from manual transactions (`6.3`)
...
- User can receive a lock-screen card nudge when entering a known geofenced place (`P2.7b`)
- User can navigate from a lock-screen nudge into the related recommendation (`P2.7b`, `P2.7`)
