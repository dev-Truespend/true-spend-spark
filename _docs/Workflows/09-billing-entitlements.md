...
| Select which plan to upgrade to (Basic or Pro) | Done | `PlanCard` is now selectable (`selected`/`onPress`); `BillingProScreen` holds `pickedPlanCode` (defaults to Pro) and the Upgrade button targets the **selected** plan instead of a single auto-computed target. Selectable = any tier above current that has a price row. |
| Test-mode checkout without real Stripe (TestFlight/MVP) | Done | `Stripe:SimulateCheckout=true` → `BillingInsertBusiness.CreateCheckoutAsync` records a trialing subscription via `SimulateCheckoutAsync` (no Stripe call, empty URL). A price **row** must still exist for the plan/period/country; the `stripe_price_id` value is unused in this mode. Flip to `false` + populate real Stripe price IDs when going live. |
...
| Start checkout | `POST /api/v1/billing/checkout` | `CreateCheckoutSessionRequest`: `planCode`, `periodCode`, `returnContextCode` ∈ {`billing`, `onboarding`} -> `HostedBillingResponse.url`. Server resolves deep-link `success_url`/`cancel_url` from `returnContextCode`: `billing` → screen 8.3, `onboarding` → Home. | Sync API | Stripe checkout lifecycle is provider-driven (see [webhook-handlers.md](webhook-handlers.md#stripe-event-mapping)) | Read/write `billing.stripe_customers`; read `billing.plan_prices` | Do not cache checkout URL. When `Stripe:SimulateCheckout=true`, skips Stripe and records a trialing subscription (test/MVP). |
