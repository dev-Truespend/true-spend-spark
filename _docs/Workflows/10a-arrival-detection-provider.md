...
| `GeoArrivalRequest` (new, transport) | `eventId`, `eventType`, `placeName`, `providerPlaceId`, `lat`, `lng`, `accuracyMeters`, `occurredAt`, `matchConfidence`, `dwellSeconds`, `movementState` | Custom-path request body. `dwellSeconds`/`movementState` (`on_foot` \| `in_vehicle` \| `still` \| `unknown`) feed candidate ranking. **Carries no user identity** — `userId` is derived server-side from JWT claims (see below) |
...
| Place density / shared parking lot | server (count within radius) | many merchants = ambiguous |
...
|---|---|---|
| **High** | Standalone store, top candidate clearly closest (Chipotle 35 m, next 180 m) | Send the lock-screen push |
| **Medium** | Strip mall, top two close (Chipotle 55 m, Starbucks 60 m, Target 75 m) | **No exact push** — surface a foreground in-app "nearby best cards" list instead (foreground path, [03-recommendations.md](03-recommendations.md)) |
| **Low** | Mall / dense lot (80 merchants in 200 m), or accuracy > 75 m | Log arrival only; wait for a stronger signal |

A wrong lock-screen push is worse than no push, so ambiguity always resolves to the in-app list or a log-only — never a guessed merchant. `GeoArrivalRequest.matchConfidence` carries the device's own score; the server re-scores with the signals above and makes the final call.
...
- `custom` medium-confidence (ambiguous top candidates / shared lot): **no lock-screen push** — surface the foreground in-app "nearby best cards" list instead.
