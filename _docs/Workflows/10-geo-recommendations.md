...
|---|---|---|---|---|---|
| Pending | Privacy Disclosure | Workflow | Foursquare receives anonymized geofence pings; **and** personal-place detection (item 6) now persists inferred recurring-dwell coordinates (home/work) in `finance.personal_places`. Neither is mentioned in [13-privacy-data.md](13-privacy-data.md) or marketing privacy copy | Cover both the third-party geofence pings and the on-server inferred-location store (purpose, retention, that it powers push *suppression*, and account-deletion purge) in the privacy policy doc | Compliance + user trust — inferred home/work location is sensitive |
