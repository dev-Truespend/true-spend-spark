...
| User can see the nearby best card live when opening the app (foreground, no push) | Done | Foreground sibling of the arrival push, owned by [03-recommendations.md](03-recommendations.md): `POST /api/v1/recommendations/nearby` reuses the same `GeoPlaceMatchBusiness` coordinate match (foreground needs only `authorized_when_in_use`, not `always`), then the shared best-card builder — minus the notification/gating. High/Medium surface a card; Low/None fall back to the home last-visited replay. Independent of the blocked Foursquare SDK. |
...
| Pending (deferred) | Home / Recurring-Dwell Suppression | Service | `GeoPlaceMatchBusiness.ScoreTier` gates High on an absolute-proximity check (`HighConfidenceProximityMeters`), but has no notion of the user's home/recurring dwell — a store genuinely within ~40m of home still scores High and pushes when the user is simply at home | Suppress arrival pushes inside a per-user recurring-dwell zone, auto-detected from repeated overnight `custom`-path arrivals (no new UI/input) | Avoids "best card at \<store\>" pushes while the user sits at home next to that store; explicitly deferred — the proximity gate covers the common "near but not at" case, build only if false pushes persist once real push (Expo) is live |
