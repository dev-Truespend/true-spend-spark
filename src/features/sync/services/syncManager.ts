...
import { SyncQueueRecord, addToSyncQueue, getSyncQueue, removeSyncQueueItem, incrementSyncRetries } from '@/shared/lib/db/indexedDB';
import { supabase } from '@/integrations/supabase/client';
import { ErrorHandler } from '@/shared/lib/errors/errorHandler';
import { measureAsync, performanceMonitor } from '@/shared/lib/performance/performanceMonitor';
...
class SyncManager {
  ...
  public async sync(): Promise<{ success: number; failed: number }> {
    ...
    try {
      const { initDB } = await import('@/shared/lib/db/indexedDB');
